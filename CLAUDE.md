...
The plugin is **webkit-only by design**: the button is injected solely by the webkit bundle, which runs inside the Steam community browser. The Plugin Database maintainers explicitly require this shape — an earlier revision had a CDP (Chrome DevTools Protocol) fallback injection path with a mode selector and injector badge, and it was rejected ("for a plugin like this, you'd simply need a webkit"). Do not reintroduce CDP injection, `window.MILLENNIUM_API` raw access, or custom-styled settings UI.

**`frontend/index.tsx`** — Millennium plugin entrypoint. Registers the plugin via `definePlugin` from `@steambrew/client` (the definition callback is async: it awaits `initSettings()` before returning the panel) and renders the settings panel (the CSStats icon) using **Steam native components only** (`ToggleField`). The store review requires native components for settings UI.
...
**`webkit/index.tsx`** — the injection path. Runs inside the Steam community browser (`steamcommunity.com`): URL guard → read settings via `callable('GetSettings')` from `@steambrew/webkit` → call `csstatsInjectMain(openExternal)`.

**`webkit/inject.ts`** — `csstatsInjectMain(openExternal)`, vanilla DOM injection (React is not available in the community browser). A normal module with real imports.
...
- The button href defaults to `steam://openurl_external/{url}` (NOT a plain `https://` link) when `openExternal` is on. If the target site is behind Cloudflare, its challenge blocks Steam's in-app CEF browser; opening in the system browser avoids it.
